...
function setupEvents() {
    ...
    document
        ...
        .addEventListener("click", newSequence);
}
...
function advanceSequence() {
    ...
    for (
        let i = 0;
        i < specs.peekables &&
        sequence.position + i < sequence.coordinates.length;
        i++
    ) {
...
function showResults() {
    ...
    document.getElementById("results-sent").style.display = performance.hash
        ? "block"
        : "none";

    document.querySelector("#results-age").innerHTML = performance.age;
    document.querySelector("#results-successes").innerHTML =
        performance.successes;
    document.querySelector("#results-failures").innerHTML =
        performance.failures;
    document.querySelector("#results-mistakes").innerHTML =
        performance.mistakes;
    document.querySelector("#results-time").innerHTML = Math.floor(
        performance.elapsed,
    );
}
...
function gridClick(evt) {
    ...
    let interaction = {
        ...
        y: y,
        distance: Math.sqrt(
            (xGoal - x) * (xGoal - x) + (yGoal - y) * (yGoal - y),
        ),
...
function validateLogin() {
    ...
    if (!age.match(/^\d+$/)) {
        document.getElementById("error-age").innerText =
            "Wrong age format: age must consist of digits only (0-9)";
    ...
    if (performance.age <= 6) {
        document.getElementById("error-age").innerText =
            "You must be at least 7 to play!";
        document.getElementById("error-age").style.display = "block";
        return false;
    }
