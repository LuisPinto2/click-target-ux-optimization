...
document.addEventListener("DOMContentLoaded", () => {
    const success = new Audio("assets/audio/successsound.mp3");
    const wrong = new Audio("assets/audio/wrongsound.mp3");
    success.preload = "auto";
    wrong.preload = "auto";

    document.querySelectorAll(".target").forEach((target) => {
        target.addEventListener("click", () => {
            if (target.classList.contains("goal-0")) {
                success.currentTime = 0; // recomeça o som se já estiver a tocar
                success.play();
            } else {
                wrong.currentTime = 0;
                wrong.play();
